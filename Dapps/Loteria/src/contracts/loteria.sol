...
contract loteria is ERC20, Ownable {
    ...
    mapping(uint256 => address) ADNBoleto;
    // Numero aleatorio
    uint256 randNonce = 0;
    // Boletos de la loteria generadors
    uint256[] boletosComprados;

    // Compra de boletos de loteria
    function compraBoleto(uint256 _numBoletos) public {
        ...
        for (uint256 i = 0; i < _numBoletos; i++) {
            ...
            boletosNFTs(usuario_contract[msg.sender]).mintBoleto(
                msg.sender,
                random
            );
        }
    ...
    function tusBoletos(address _propietario)
        public
        view
    ...
    function generarGanador() public onlyOwner {
        ...
        uint256 longitud = boletosComprados.length;
        ...
        uint256 eleccion = boletosComprados[random];
